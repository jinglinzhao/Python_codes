...
class Model2(Model):
    ...
    def get_value(self, t):
        ...
        # Planet 2
        M_anom2 = 2*np.pi/np.exp(self.P1*10) * (t - 100*self.tau2)
        e_anom2 = solve_kep_eqn(M_anom2, self.e2)
        f2      = 2*np.arctan( np.sqrt((1+self.e2)/(1-self.e2))*np.tan(e_anom2*.5) )
        rv2     = 100*self.k2*(np.cos(f2 + self.w2) + self.e2*np.cos(self.w2))
...
def lnprior(theta):
    ...
    if (3. < P1 < 8.) and (0 < tau1 < 20) and (0 < k1 < 0.1) and (-2*np.pi < w1 < 2*np.pi) and (0 < e1 < 0.9) and \
       (7. < P2 < 12.) and (0 < k2 < 0.1) and (-2*np.pi < w2 < 2*np.pi) and (0 < e2 < 0.9):
        return 0.0
    return -np.inf
...
pos = [[6., 1., np.log(np.std(y))/100, 0, 0.1,\
        9., 1., np.log(np.std(y))/100, 0, 0.1, 0., 1] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)] 
...
real_samples[:,1:3] = 100*real_samples[:,1:3]
real_samples[:,6:8] = 100*real_samples[:,6:8]
