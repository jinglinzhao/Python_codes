...
shutil.copy('HD85390-2_planet_0_jitter_2sets.py', str(time0)+'/HD85390-2_planet_0_jitter_2sets.py')  
os.chdir(str(time0))
...
def lnprior(theta):
    ...
    if (3. < P1 < 10.) and (-1. < tau1 < 3.) and (0 < k1 < 0.1) and (-2*np.pi < w1 < 2*np.pi) and (0 < e1 < 0.9) and \
       (-20 < tau2 < 30) and (0. < k2 < 0.2) and (-2*np.pi < w2 < 2*np.pi) and (0 < e2 < 0.9):
        return 0.0
    return -np.inf
...
pos = [[7., 1., np.log(np.std(y))/100, 0, 0.4,\
        120., 1., np.log(np.std(y))/100, 0, 0.4, 0., 0.] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)] 
...
import corner
labels=[r"$P1$", r"$T_{1}$", r"$K1$", r"$\omega1$", r"$e1$", r"$P2$", r"$T_{2}$", r"$K2$", r"$\omega2$", r"$e2$", "offset1", "offset2"]
