...
jitter_smooth100 = np.loadtxt('jitter_smooth100.txt')
...
shutil.copy('HD85390-1_planet_high_e.py', str(time0)+'/HD85390-1_planet_high_e.py')  
os.chdir(str(time0))
...
#==============================================================================
class Model(Model):
    parameter_names = ('P1', 'tau1', 'k1', 'w1', 'e1', 'offset', 'alpha')
    ...
    def get_value(self, t):
        ...
        return rv1 + self.offset + self.alpha * jitter_smooth100

class Model2(Model):
    parameter_names = ('P1', 'tau1', 'k1', 'w1', 'e1', 'offset')
...
def lnprior(theta):
    ...
    if (3. < P1 < 10.) and (0 < tau1 < 20) and (0 < k1 < 0.1) and (-2*np.pi < w1 < 2*np.pi) and (0 < e1 < 0.9):
        return 0.0
    return -np.inf
...
pos = [[6., 1., np.log(np.std(y))/100, 0, 0.1, 0., 1] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)] 
...
P1, tau1, k1, w1, e1, offset, alpha = aa[:,0]
fit_curve   = Model(P1=P1/100, tau1=tau1/100, k1=k1/100, w1=w1, e1=e1, offset=offset, alpha=alpha)
y_fit       = fit_curve.get_value(x)

fit_curve2  = Model2(P1=P1/100, tau1=tau1/100, k1=k1/100, w1=w1, e1=e1, offset=offset)
...
plt.plot(x, alpha*jitter_smooth100, 'ro', alpha=.5, label='smoothed jitter')
