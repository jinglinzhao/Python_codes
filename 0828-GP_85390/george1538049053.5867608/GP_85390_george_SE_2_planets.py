...
y       = all_rvs[:,1]
yerr 	= all_rvs[:,2]
# jitter_smooth 	= np.loadtxt('./data/jitter_raw.txt')
# fwhm 			= np.loadtxt('./data/hd85390_fwhm.dat')
# from functions import gaussian_smoothing
# t_resample = np.linspace(min(t), max(t), 1000)
# fwhm_smooth = gaussian_smoothing(fwhm[:,0], fwhm[:,1], fwhm[:,0], 200., np.ones(len(fwhm[:,0])))
# plt.figure()
# # plt.plot(jitter_smooth, fwhm[:,1])
# plt.plot(fwhm[:,0], fwhm[:,1], '.')
# plt.plot(t_resample, fwhm_smooth)
# plt.show()

# plt.figure()
# plt.plot(jitter_smooth, fwhm_smooth[0:96], '.')
# plt.show()
