...
for n in range(N):
    ...
    # class Model(Model):
    #     parameter_names = ('a', 'nu', 'phi', 'k', 'offset')

    #     def get_value(self, t):
    #         return self.k*RV_X - (self.k-1)*self.a*np.sin(t/100. * self.k * 2. * np.pi + self.phi) + self.offset
    # y = RV_X

    def lnprior(theta):
        a, nu, phi, ky, kz, offset_Y, offset_Z = theta
        if (0 < a < 5) and (0 < nu < 5) and (-2*np.pi < phi < 2*np.pi) and (0<ky<1) and (1<kz<5):
            return 0.0
        return -np.inf
    ...
    def lnlike(theta, xmc, ymc, yerrmc):
        a, nu, phi, ky, kz, offset_Y, offset_Z = theta
        # model_Y = Model(a=a, nu=nu, phi=phi, k=ky, offset=offset_Y).get_value(x[0:100])
        # model_Z = Model(a=a, nu=nu, phi=phi, k=kz, offset=offset_Z).get_value(x[100:200])
        # return -0.5*( 0.1*sum( ((y[0:100]-model_Y)/yerr[0:100])**2.) + 0.9*sum(((y[100:200]-model_Z)/yerr[100:200])**2.) )
        vy = ymc[0:40]
        vz = ymc[40:80]
        model_Y = ky*RV_X - (ky-1)*a * np.sin(x/100. * nu * 2. * np.pi + phi) + offset_Y
        model_Z = kz*RV_X - (kz-1)*a * np.sin(x/100. * nu * 2. * np.pi + phi) + offset_Z 
    ...
    print("Running first burn-in...")
    pos         = [[np.std(RV_X), 1, 0, 0.8, 1.5, 0, 0] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)] 
    ...
    fig, axes = plt.subplots(ndim, figsize=(10, 7), sharex=True)
    labels=[r"$A$", r"$\nu$", r"$\omega$", "ky", "kz", "offset_Y", "offset_Z"]
    for i in range(ndim):
        ax = axes[i]
    ...
    import corner
    fig = corner.corner(real_samples, labels=labels, quantiles=[0.16, 0.5, 0.84], show_titles=True, title_kwargs={"fontsize": 12})
    ...
    # Statistics
    #==============================================================================    
    a, k, phi, ky, kz, offset_Y, offset_Z = map(lambda v: np.array(v), zip(*np.percentile(real_samples, [50, 16, 84, 2.5, 97.5], axis=0)))
    ...
    print(np.vstack((a, k, phi, ky, kz, offset_Y, offset_Z)))
