...
for n in range(N):
    ...
    pos2     = [[np.log(np.var(RV_X)), np.log(1), 1., 0.] + 1e-2*np.random.randn(ndim) for i in range(nwalkers)] 
    ...
    if (mode == 5):
        ...
        print("Running first burn-in...")
        pos         = [[np.var(RV_X), 1, 0, 0.8, 0] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)] 
        ...
        import corner
        fig = corner.corner(real_samples, labels=[r"$A$", r"$\nu$", r"$\omega$", "kz", "offset_Z"], truths=[real_a, real_k, real_phi, 100, 100],
                            quantiles=[0.16, 0.5, 0.84], show_titles=True, title_kwargs={"fontsize": 12})
        ...
        # Statistics
        #==============================================================================    
        a, k, phi, kz, offset_Z = map(lambda v: np.array(v), zip(*np.percentile(real_samples, [50, 16, 84, 2.5, 97.5], axis=0)))
