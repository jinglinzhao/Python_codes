...
plt.plot(RV_jitter, GG-YY, 'd', label=r'$\Delta RV_{L} (\rho=%.2f, k=%.2f)$' %(r1, fit1[0]), alpha=alpha)
plt.plot(RV_jitter, ZZ-GG, 'P', label=r'$\Delta RV_{H} (\rho=%.2f, k=%.2f)$' %(r2, fit2[0]), alpha=alpha)
plt.axis('scaled')
plt.ylim(-1.8, 2.8)
plt.title(r'$K = 0$ m/s')
