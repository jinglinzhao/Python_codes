...
ax = plt.subplot(131)
fit1 = np.polyfit(RV_jitter, XX-YY, 1)
fit2 = np.polyfit(RV_jitter, ZZ-XX, 1)
r1, p = stats.pearsonr(RV_jitter, XX-YY)
r2, p = stats.pearsonr(RV_jitter, ZZ-XX)
plt.plot(RV_jitter, XX-YY, 'd', label=r'$\Delta RV_{L} (\rho=%.2f, k=%.2f)$' %(r1, fit1[0]), alpha=alpha)
plt.plot(RV_jitter, ZZ-XX, 'P', label=r'$\Delta RV_{H} (\rho=%.2f, k=%.2f)$' %(r2, fit2[0]), alpha=alpha)
plt.axis('scaled')
plt.ylim(-1.8, 2.8)
plt.title(r'$K = 0$ m/s')
