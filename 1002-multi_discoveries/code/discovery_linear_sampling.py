...
for kk in np.arange(10):
    ...
    class Model2(Model):
        ...
        def get_value(self, t):
            ...
            # Planet 1
            M_anom1 = 2*np.pi/(self.P1) * (t - self.tau1)
            e_anom1 = solve_kep_eqn(M_anom1, self.e1)
            f1      = 2*np.arctan( np.sqrt((1+self.e1)/(1-self.e1))*np.tan(e_anom1*.5) )
    ...
    def lnprior(theta):
        ...
        if (-2*np.pi < w1 < 2*np.pi) and (0 < e1 < 0.9) and (alpha > 0.1):
            return 0.0
        return -np.inf
    ...
    INIT_P =  np.random.uniform(0,20)
    INIT_tau = np.random.uniform(0,2)
    INIT_e =  np.random.uniform(0.01,0.89)
    print("Running first burn-in...")
    pos = [[INIT_P, INIT_tau, np.std(XX)/100, 0., INIT_e, 0., 0.3] + 1e-2*np.random.randn(ndim) for i in range(nwalkers)] 
    ...
    labels_log=[r"$\frac{P_{1}}{100}$", r"$\frac{T_{1}}{1000}$", r"$\frac{K_{1}}{100}$", r"$\omega1$", r"$e1$", "offset1", r"$\alpha$"]
    for i in range(ndim):
        ax = axes[i]
    ...
    # Jitter#
    Jitter      = Model(P1=P1/100, tau1=tau1/1000, k1=0, w1=w1, e1=e1, offset1=0, alpha=alpha/10)
    y_jitter    = Jitter.get_value(t)
    plt.plot(t, y_jitter, 'ro', alpha=.5, label='smoothed jitter')

    Fit         = Model(P1=P1/100, tau1=tau1/1000, k1=k1/100, w1=w1, e1=e1, offset1=offset1, alpha=alpha/10)
