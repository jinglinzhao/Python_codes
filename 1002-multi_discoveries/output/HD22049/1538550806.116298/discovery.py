...
pos = [[2., 1., np.std(XX)/100, 0, 0.4, 0, 3] + 1e-2*np.random.randn(ndim) for i in range(nwalkers)] 
...
real_samples[:,0:2] = 1000*real_samples[:,0:2]
...
labels_log=[r"$\frac{P_{1}}{1000}$", r"$\frac{T_{1}}{1000}$", r"$\frac{K_{1}}{100}$", r"$\omega1$", r"$e1$", "offset1", r"$\alpha$"]
for i in range(ndim):
    ax = axes[i]
